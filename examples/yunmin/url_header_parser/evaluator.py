...
EXPERIMENT_NAME = "url_header_parser"
...
def _load_baseline() -> dict:
    ...
    return {
        ...
        "p99_latency_us": 50.0,
        "off_cpu_ratio": 0.02,
        "bcoz_max_speedup": 4.0,
    }


def _set_baseline(baseline: dict) -> None:
    Baseline.THROUGHPUT_OPS_SEC = float(baseline.get("ops_per_sec", 20000.0))
    Baseline.P99_LATENCY_US = float(baseline.get("p99_latency_us", 50.0))
    Baseline.OFF_CPU_RATIO = float(baseline.get("off_cpu_ratio", 0.02))
    Baseline.BCOZ_MAX_SPEEDUP = float(baseline.get("bcoz_max_speedup", 4.0))
...
def evaluate(program_path: str) -> dict:
    ...
    try:
        metrics_path = Path(tempfile.mkdtemp(prefix="aiopt_url_")) / "metrics.json"
