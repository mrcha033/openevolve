...
def main():
    parser = argparse.ArgumentParser(description="N-Body benchmark runner")
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="nbody_bench_"))
    ...
    run_cmd = [str(bin_path), "--json", str(metrics_path),
               "--seed", str(args.seed), "--bodies", str(args.bodies),
