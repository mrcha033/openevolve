...
def _set_baseline(baseline: dict) -> None:
    Baseline.THROUGHPUT_OPS_SEC = float(baseline.get("ops_per_sec", 290.0))
    Baseline.P99_LATENCY_US = float(baseline.get("p99_latency_us", 5100.0))
...
def evaluate(program_path: str) -> dict:
    ...
    try:
        tmp_dir = Path(tempfile.mkdtemp(prefix="aiopt_nbody_"))
