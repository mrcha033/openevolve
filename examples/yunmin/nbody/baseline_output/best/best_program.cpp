...
static void ComputeForces(const std::vector<Body> &bodies,
    ...
    // Highly optimized loop with reduced operations and better instruction scheduling
    // Precompute common values to minimize redundant calculations
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = i + 1; j < n; ++j) {
            ...
            // Precompute shared factor to avoid redundant multiplications
            double shared_factor = bj.mass * inv_r3;
            
            // Apply forces to both bodies in a way that maximizes instruction level parallelism
            // and reduces dependency chains
            fx[i] += dx * shared_factor;
            fx[j] -= dx * mi * inv_r3;
            fy[i] += dy * shared_factor;
            fy[j] -= dy * mi * inv_r3;
            fz[i] += dz * shared_factor;
            fz[j] -= dz * mi * inv_r3;
        }
    }
}
