...
def _set_baseline(baseline: dict) -> None:
    Baseline.THROUGHPUT_OPS_SEC = float(baseline.get("ops_per_sec", 30000.0))
    Baseline.P99_LATENCY_US = float(baseline.get("p99_latency_us", 40.0))
...
def evaluate(program_path: str) -> dict:
    ...
    try:
        metrics_path = Path(tempfile.mkdtemp(prefix="aiopt_hash_")) / "metrics.json"
